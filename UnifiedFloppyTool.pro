...
qtHaveModule(serialport) {
    QT += serialport
    DEFINES += UFT_HAS_SERIALPORT
    # MF-249/MF-252: hardwaretab.cpp + the QSerialPort transports use
    # UFT_HAS_QSERIALPORT (same naming as the CMake build) to gate
    # production-transport branches for Applesauce + ADFCopy.
    DEFINES += UFT_HAS_QSERIALPORT
...
SOURCES += \
    ...
    src/hardware_providers/usbfloppy_provider_v2.cpp \
    src/hardware_providers/teensy_probe.cpp \
    src/hardware_providers/qprocess_subprocess_runner.cpp \
    src/hardware_providers/ufi_runners.cpp

# MF-249/MF-252: QSerialPort-backed Applesauce + ADFCopy transports
# and their runner-factories. Compile only when the Qt SerialPort
# module is available — without it, these TUs fail to compile
# (#include <QSerialPort>). The providers themselves keep working
# as honest-stubs (nullptr-runner construction site stays in
# hardwaretab.cpp's #else branch).
qtHaveModule(serialport) {
    SOURCES += \
        src/hardware_providers/qserial_applesauce_transport.cpp \
        src/hardware_providers/applesauce_serial_runners.cpp \
        src/hardware_providers/qserial_adfcopy_transport.cpp \
        src/hardware_providers/adfcopy_serial_runners.cpp
    HEADERS += \
        src/hardware_providers/qserial_applesauce_transport.h \
        src/hardware_providers/applesauce_serial_runners.h \
        src/hardware_providers/qserial_adfcopy_transport.h \
        src/hardware_providers/adfcopy_serial_runners.h
}
