...
| P1.24 | **P0-on-integration** — KryoFlux+FluxEngine: echter Flux-Decode statt undekodierte Backend-Container-Bytes in `FluxCaptured::transitions_ns` | P1.17 | 🔄 **Schritt 1 (MF-203):** Integritäts-Verstoß gestoppt — `do_read_raw_flux` gibt ehrlichen `ProviderError` statt fehletikettierter Bytes. **Schritt 2 — KryoFlux ✅ (MF-208):** echter Stream-Decoder `src/flux/uft_kryoflux_stream.c` implementiert `uft_kf_decode()` nach der dokumentierten KryoFlux-Stream-Spec (Flux1/2/3 + Nop1/2/3 + Ovl16 + OOB Index/StreamInfo/KFInfo/StreamEnd/EOF); `KryoFluxProviderV2::do_read_raw_flux` dekodiert jetzt → `FluxCaptured` mit ns-Intervallen + gemessenen `index_times_ns`, truncated → `FluxMarginal`. Decoder fabriziert nie einen Flux-Wert. Tests: `test_kryoflux_stream.c` (11 synthetische Spec-Vektoren, exakte Tick-Level-Asserts) + `test_kryoflux_provider_v2` Stream-Decode-Smoke — beide grün. Fehletikettierter Block-Typ-Enum in `uft_kryoflux.h` korrigiert. **Verbleibend — FluxEngine `.flux`:** analoger Decoder noch offen. **Hinweis:** synthetische Vektoren beweisen „Decoder == Spec-Lesart", nicht „== echte Hardware" — HIL-Capture-Vektoren bleiben Folgearbeit. |
